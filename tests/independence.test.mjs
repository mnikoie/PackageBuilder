...
describe("استقلال از محیط", () => {
  ...
  test("هیچ وابستگیِ npmای ندارد (تا قدمِ ۵)", () => {
    const pkg = JSON.parse(readFileSync(join(ROOT, "package.json"), "utf8"));
    assert.equal(pkg.dependencies, undefined, "dependencies اضافه شده — عمدی بود؟");
    assert.equal(pkg.devDependencies, undefined, "devDependencies اضافه شده — عمدی بود؟");
  });
  ...
  test("همهٔ importها نسبی یا از خودِ Node هستند", () => {
    const offenders = [];
    for (const file of [...sourceFiles(join(ROOT, "src")), ...sourceFiles(join(ROOT, "tests"))]) {
      const text = readFileSync(file, "utf8");
    ...
    assert.deepEqual(offenders, [], `importِ بیرونی:\n${offenders.join("\n")}`);
  });
});
